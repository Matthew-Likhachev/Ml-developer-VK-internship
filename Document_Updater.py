...
class Processor:
    ...
    def process(self, document: TDocument) -> TDocument:
        ...
        if not document.pub_date:
            raise ValueError("Document publish date cannot be empty")
        if not isinstance(document.pub_date, int):
            raise ValueError("Document publish date must be int")

        if not document.fetch_time:
            raise ValueError("Document fetching time cannot be empty")
